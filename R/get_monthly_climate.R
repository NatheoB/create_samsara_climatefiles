...
get_monthly_climate <- function(coords,
  ...
  # Search for all years to fetch for each site (be careful, chelsa only between 1983 and 2018)
  years_to_fetch <- coords %>% 
    dplyr::mutate(n_years = year_max - (year_min-2) + 1) %>% 
    tidyr::uncount(n_years) %>% 
    dplyr::group_by(id) %>% 
    dplyr::mutate(year = seq(unique(year_min-2), unique(year_max))) %>% 
    dplyr::filter(year >= 1989 & year <= 2018)
  
  years <- unique(years_to_fetch$year)
  
  ### Extract climate data ----
  out <- sapply(vars,  
                function(x) setNames(vector("list", length(years)), years),
                USE.NAMES = T, simplify = F)
  
  print("Fetching climate data from Chelsa...")
  pb <- txtProgressBar(min = 0, max = length(years)*length(vars), style = 3)
  i <- 0
  ...
  out <- out %>%
    ...
    # Bind variables into a single dataframe
    purrr::reduce(left_join, by = c("id", "year", "month")) %>%
